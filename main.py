...
def getDinner(): 
    dinner = int(input("Please enter the amount of dinners: "))
    while True:
        if dinner > len(fortunes):
            dinner = int(input("Sorry we only have up to 22 fortunes available please pick a lower number: "))
        else:
            break
    return dinner
